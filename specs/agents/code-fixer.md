...
tasks:
  - id: fix-lint
    description: Fix golangci-lint errors following error handling priority
    type: action
    required: true
  ...
  - id: verify
    description: Re-run validation to confirm fixes
    type: command
    command: "golangci-lint run && go test -v ./..."
    required: true
---
