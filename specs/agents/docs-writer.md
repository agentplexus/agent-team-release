...
requires: [schangelog, git]
tasks:
  - id: parse-commits
    description: Parse commits since last tag using schangelog
    type: command
    command: "schangelog parse-commits --since=<previous-tag>"
    required: true

  - id: update-changelog-json
    description: Update CHANGELOG.json with new version entry
    type: action
    required: true
  ...
  - id: create-release-notes
    description: Create docs/releases/vX.Y.Z.md
    type: action
    required: true

  - id: update-mkdocs
    description: Update mkdocs.yml nav with new version
    type: action
    required: false

  - id: verify
    description: Validate all documentation updates
    type: command
